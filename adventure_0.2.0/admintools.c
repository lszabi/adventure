...
const int item_fields_n = 1;
const field_t item_fields[] = {
	{
		.name = "name",
		.size = sizeof(char) * 16,
		.type = FIELD_CHAR,
...
int main(int argc, char *argv[]) {
	...
	for ( int i = 0; i < n; i++ ) {
		if ( char_eq(operations[i], "players") ) {
			op_players = 1;
		} else if ( char_eq(operations[i], "items") ) {
			op_items = 1;
		} else {
	...
	if ( char_eq(argv[1], "import") ) {
		if ( op_players ) {
			import_db("players", (void **)&player_database, sizeof(player_database_t), player_fields, player_fields_n, &player_database_n);
		}
		if ( op_items ) {
			import_db("items", (void **)&item_database, sizeof(item_database_t), item_fields, item_fields_n, &item_database_n);
			file_header_t head = { .block_count = item_database_n, .block_size = sizeof(item_database_t) };
			FILE *file = fopen("db/items.bin", "wb");
			if ( file == NULL ) {
				printf("File write error.\n");
				return 1;
			}
			if ( fwrite(&head, sizeof(file_header_t), 1, file) != 1 ) {
				fclose(file);
				printf("File write error.\n");
				return 1;
			}
			if ( fwrite(item_database, head.block_size, head.block_count, file) != head.block_count ) {
				fclose(file);
				printf("File write error.\n");
				return 1;
			}
		...
		if ( op_items ) {
			export_db("items", item_database, sizeof(item_database_t), item_fields, item_fields_n, item_database_n);
		}
	}
	if ( database_close() < 0 ) {
