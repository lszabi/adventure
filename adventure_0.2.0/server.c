		...
		if ( char_eq(cmd, "close") || char_eq(cmd, "stop") ) {
			...
			if ( sub && char_eq(sub, "player") ) {
				sprintf(out, " %-32s - view player profile\n"
					...
					" %-32s - list player's invertory\n"
					" %-32s - give item to player",
					"player {name} profile", "player {name} ban", "player {name} rcon", 
					"player {name} teleport {x} {y}", "player {name} send {str}", "player {name} inv", 
					"player {name} give {item} {quantity}"
				);
				...
				sprintf(out, " %-32s - stop server\n"
					...
					" %-32s - broadcast msg",
					"stop/close", "help player", "help list", "search {database} {str}", "print {msg}", "broadcast {msg}"
				);
			}
			...
			} else if ( char_eq(sub, "inv") ) {
				// List invertory
				player_list_invertory(&player_database[idx], out);
			} else if ( char_eq(sub, "give") && arg ) {
				char *q_str = str_explode_next(arg, ' ');
				int err = 0;
				int q = tosint(q_str, &err);
				int item_id = get_item(arg);
				if ( item_id > -1 && !err ) {
					player_give_item(idx, item_id, q, name);
					sprintf(out, "%sGiven %dx %s to %s.", CL_FRONT_GREEN, q, item_database[item_id].name, player_database[idx].name);
				} else {
					sprintf(out, "%sFailed to give %dx %s to %s.", CL_FRONT_RED, q, arg, player_database[idx].name);
				}
			} else {
			...
			} else if ( char_eq(sub, "items") ) {
				for ( int i = 0; i < item_database_n; i++ ) {
					if ( strncasecmp(item_database[i].name, start, strlen(start) ) == 0 ) {
						sprintf(out + strlen(out), "%s, ", item_database[i].name);
					}
				}
			} else {
				sprintf(out, "Databases: players, items");
			}
...
int process_response(player_info_t *player, packet_t *packet) {
	...
	if ( player->stage == STAGE_VERSION ) {
		...
		if ( packet->type == AD_PACKET_CMD ) {
			if ( packet->cmd.cmd == AD_CMD_PLAYER ) {
				...
				if ( packet->cmd.arg[0] == '\0' ) {
					...
					if ( idx != -1 && player_can_meet(idx, player->player_db) ) {
						...
						error_packet(packet, AD_ERROR_SENDMSG, AD_ERROR_FLAG_MSG, CL_FRONT_RED "Failed to show profile. ( Player not found or not in the same area )");
					}
				}
			...
			} else if ( packet->cmd.cmd == AD_CMD_INVERTORY ) {
				// List invertory
				char buf[2048];
				player_list_invertory(&player_database[player->player_db], buf);
				player_send_fragments(player, buf, packet);
			} else if ( packet->cmd.cmd == AD_CMD_GIVE ) {
				// Give item
				char *item = packet->cmd.arg;
				char *name = str_explode_next(item, ';');
				int player_id = get_player(name);
				int item_id = get_item(item);
				int q = packet->cmd.arg_n[0];
				if ( q > 0 && player_id != -1 && item_id != -1 && player_can_meet(player_id, player->player_db) 
					&& q <= player_count_item(player->player_db, item_id)
					&& player_give_item(player_id, item_id, q, player_database[player->player_db].name) 
				) {
					player_give_item(player->player_db, item_id, -q, NULL);
					error_packet(packet, AD_ERROR_OK, AD_ERROR_FLAG_MSG, NULL);
					sprintf(packet->error.msg, "%sYou gave %dx %s to %s.", CL_FRONT_GREEN, q, item_database[item_id].name, player_database[player_id].name);
				} else {
					error_packet(packet, AD_ERROR_GIVE, AD_ERROR_FLAG_MSG, NULL);
					sprintf(packet->error.msg, "%sFailed to give item.", CL_FRONT_RED);
				}
			}
		} else if ( packet->type == AD_PACKET_TASK ) {
			int task = player_database[player->player_db].task.type;
			if ( packet->task.type == AD_TASK_IDLE ) { // abort task
				if ( task == AD_TASK_TRAVEL ) {
					double time_needed = ( get_player_travel_distance(player->player_db) * 3600 ) / player_database[player->player_db].movement_speed;
					double time_elapsed = get_time() - player_database[player->player_db].task.started;
					double time_rem = time_needed - time_elapsed;
					int x1 = player_database[player->player_db].x;
					int y1 = player_database[player->player_db].y;
					int x2 = player_database[player->player_db].task.travel.x;
					int y2 = player_database[player->player_db].task.travel.y;
					double x3 = ( time_rem * x1 + time_elapsed * x2 ) / ( time_elapsed + time_rem );
					double y3 = ( time_rem * y1 + time_elapsed * y2 ) / ( time_elapsed + time_rem );
					int x = x1 > x3 ? ceil(x3) : floor(x3);
					int y = y1 > y3 ? ceil(y3) : floor(y3);
					player_move(player->player_db, x, y);
				}
			} else {
				if ( task != AD_TASK_IDLE ) {
					error_packet(packet, AD_ERROR_TASK, AD_ERROR_FLAG_MSG, CL_FRONT_RED "You are doing another task at the moment.");
				} else {
					if ( packet->task.type == AD_TASK_TRAVEL ) {
						task_t *player_task = &player_database[player->player_db].task;
						player_task->type = AD_TASK_TRAVEL;
						player_task->travel.x = packet->task.travel.x;
						player_task->travel.y = packet->task.travel.y;
						player_task->started = get_time();
					} else {
						error_packet(packet, AD_ERROR_TASK, AD_ERROR_FLAG_MSG, CL_FRONT_RED "Invalid task.");
					}
				}
			}
		} else {
...
void process_player_task(player_info_t *player) {
	int t = get_time();
	if ( player->last_process + PROCESS_DELAY <= t ) {
		int task = player_database[player->player_db].task.type;
		if ( task == AD_TASK_TRAVEL ) {
			double d = get_player_travel_distance(player->player_db);
			int time_needed = ( d * 3600 ) / player_database[player->player_db].movement_speed;
			int time_started = player_database[player->player_db].task.started;
			task_travel_t *dest = &player_database[player->player_db].task.travel;
			if ( time_started + time_needed < t ) {
				player_move(player->player_db, dest->x, dest->y);
			} else {
				packet_t p;
				memset(&p, 0, sizeof(packet_t));
				p.type = AD_PACKET_TASK;
				p.task.type = AD_TASK_TRAVEL;
				p.task.travel.x = dest->x;
				p.task.travel.y = dest->y;
				p.task.time_rem = ( time_started + time_needed ) - t;
				player_send_im(player->player_db, &p);
			}
		}
		player->last_process = get_time();
	}
}
